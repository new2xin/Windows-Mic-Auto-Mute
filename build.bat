...
if exist dist\LyraAutoMute.exe (
...
dotnet build LyraAutoMute.csproj -c Release --ignore-failed-sources -p:BaseIntermediateOutputPath=buildobj\ -p:OutputPath=dist\ -p:UseSharedCompilation=false
if errorlevel 1 exit /b %errorlevel%
echo Build complete: %~dp0dist\LyraAutoMute.exe
endlocal
